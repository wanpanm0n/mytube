...
private function sendMessageBackup($originator,$message) {
	...
	public function actionBestPackage() {
		...
		// 	)
		// );
		$dataReader = Yii::app()->db->createCommand('SELECT tp.*, tc.name as country_name, tc.alpha_2, tpp.*, tpr.name as product_name from tbl_package tp inner join tbl_country tc on tp.country_id = tc.id inner join tbl_package_product tpp on tpp.package_id = tp.id inner join tbl_product tpr on tpr.id = tpp.product_id
		 where tp.is_active is true')->queryAll();
		$package = array();
		foreach( $dataReader as $row ) :
			if(!isset($package[$row['country_name']])) {
				$package[$row['country_name']] = array(
					'country' => $row['country_name'],
					'id' => $row['package_id'],
					'price' => 'RM'.$row['price'],
					'flag' => Yii::app()->getBaseUrl(true).'/images/flags100/'.$row['alpha_2'].'.png',
					'products' => array()
				);
			}
			array_push($package[$row['country_name']]['products'], array(
