...
class ApiController extends Controller {
	...
	"internetbundlerequest","changepasscode","logger","categories","myorders", "balancedetail", "getname", "subscribe");
	...
	protected function beforeAction($event) {
		...
		if(!array_key_exists("HTTP_APIKEY", $_SERVER) || (array_key_exists("HTTP_APIKEY", $_SERVER) && $_SERVER["HTTP_APIKEY"] != $this->appKey)):
			$this->apiUnauthorizedError("Access denied.");
		endif;
		...
		if(in_array($this->actionName, $this->authenticateActions)) :
			...
			($this->token = Identity::model()->find('token=:token',array('token' => $_SERVER["HTTP_X_TOKEN"]))) == null):
				$this->apiUnauthorizedError("Invalid access.Key doesnot exists.");
			else:
	...
	public function apiUnauthorizedError($message = "Unauthorized") {
...
public function actionGetFeaturedContentsWithPagination($language){
	...
	public function actionLogIn(){
		...
		if(isset($_POST)):
			...
			$otpModel = Otp::model()->find('code=:code and reference_number=:reference_number',	array('code' => $code,	'reference_number' => $reference_number));
			if(is_null($otpModel)):
...
private function sendMessageBackup($originator,$message) {
	...
	public function actionBestPackage() {
		// $package = array(
		// 	array(
		// 		'country' => 'Bangladesh',
		// 		'id' => '40929',
		...
		$dataReader = Yii::app()->db->createCommand('SELECT 
						...
						FROM
						tbl_package tp 
						INNER JOIN tbl_package_product tpp 
						ON tpp.package_id = tp.id 
						INNER JOIN tbl_product tpr 
						ON tpr.id = tpp.product_id 
						WHERE tp.is_active IS TRUE')->queryAll();
		$package = array();
		$finalPackage = array();
		foreach( $dataReader as $row ) :
			if(!isset($package[$row['title']])) {
				$package[$row['title']] = array(
					'country' => $row['title'],
					'id' => $row['package_id'],
					'price' => 'RM'.$row['price'],
					'flag' => $row['icon'],
			...
			array_push($package[$row['title']]['products'], array(
				'name' => $row['product_name'],
				'title' => $row['package_product_title'],
				'description' => $row['package_product_description']
			));
		endforeach;
		// $title = $name = $descrption = "";
		// foreach($package as $key => $eachProduct){
		// 	foreach($eachProduct['products'] as $eachProducts){
		// 		if($eachProducts['title'] != "" && $eachProducts['description'] != ""){
		// 			$title = $eachProducts['title'];
		// 			$descrption = $eachProducts['description'];
		// 			foreach($package[$key]['products'] as $subKey => $allSub){
		// 				$package[$key]['products'][$subKey]['title'] = $title;
		// 				$package[$key]['products'][$subKey]['description'] = $descrption;
		// 			}
		// 			break;
		// 		}else if($eachProducts['title'] != "" || $eachProducts['description'] != ""){
		// 			$title = $eachProducts['title'];
		// 			$descrption = $eachProducts['description'];
		// 			foreach($package[$key]['products'] as $subKey => $allSub){
		// 				$package[$key]['products'][$subKey]['title'] = $title;
		...
		// 	}
		// }
		$this->data = array_values($package);
		$this->renderJson();
	}
	...
	public function actionSubscribe()
	{
		...
		$transactionId = time();
		$simNumber =$this->sim_no;
		// $simNumber ="01141000008";

		$package = Package::model()->find('id=:id',array('id' => $packageId));
		if(!$package):
			$this->apiError("Invalid Package");
		endif;

		$sql = Yii::app()->params['externalApi'][YII_ENV].'/users/callSubscribe/'.$transactionId.'/'.$simNumber.'/'.$packageId.'/2';
		$output = Yii::app()->curl->get($sql, $params= array(),true);
		$output  = str_replace('{},','"",',$output);
		$decodeJson = json_decode($output, true);
		$decodeJson['ResultDescHeader'] = 'Error';

		if($decodeJson['ResultCode'] == '000') {
			$decodeJson['ResultDescHeader'] = 'Congratulations';
			$decodeJson['ResultDesc'] = 'You have subscribed to the best package';
			$simUserModel = Simusers::model()->find('sim_no=:sim_no',array('sim_no' => $this->sim_no));
			$model=new PackageTransaction;
			$model->attributes = array("sim_users_id"=>$simUserModel["id"],"package_id"=>$packageId);
			$model->save();
		}

		$this->data = $decodeJson;
		$this->renderJson();
	}
	...
	public function actionGetName() {
		$simNumber =$this->sim_no;
		// $simNumber ="01141000008";
		$sql = Yii::app()->params['externalApi'][YII_ENV].'/users/getName/'.$simNumber;
		$output = Yii::app()->curl->get($sql, $params= array(),true);
		$output  = str_replace('{},','"",',$output);
		$decodeJson = json_decode($output);
		$this->data = $decodeJson;
	...
	public function actionBalanceDetail(){
		...
		$transaction_id = time();
		// $simNumber ="01141000008";
		$sql = Yii::app()->params['externalApi'][YII_ENV].'/users/queryBalanceDetail/'.$simNumber;
		$output = Yii::app()->curl->get($sql, $params= array(),true);
		$output  = str_replace('{},','"",',$output);
		$decodeJson = json_decode($output, true);
		if(!empty($decodeJson['PrepaidBalance'])) {
			$decodeJson['PrepaidBalance'] = 'RM'.$decodeJson['PrepaidBalance'];
		}
		if(isset($decodeJson['BI']) && $decodeJson['BI']) {
			$biExpirtyDate =  date('d/m/Y', strtotime('last day of this month'));
			$decodeJson['BIExpireDate'] = $biExpirtyDate;
		}
		if(isset($decodeJson['ValidityDate']) && $decodeJson['ValidityDate']) {
			$formattedDate =  DateTime::createFromFormat('d/m/y', $decodeJson['ValidityDate']);
			$decodeJson['ValidityDate'] = $formattedDate->format('d/m/Y');
		}
		if(isset($decodeJson['BestDataExpireDate']) && $decodeJson['BestDataExpireDate']) {
			$formattedDate =  DateTime::createFromFormat('d/m/y', $decodeJson['BestDataExpireDate']);
			$decodeJson['BestDataExpireDate'] = $formattedDate->format('d/m/Y');
		}
		if(isset($decodeJson['IDDExpireDate']) && $decodeJson['IDDExpireDate']) {
			$formattedDate =  DateTime::createFromFormat('d/m/y', $decodeJson['IDDExpireDate']);
			$decodeJson['IDDExpireDate'] = $formattedDate->format('d/m/Y');
		}
		if(isset($decodeJson['MIExpireDate']) && $decodeJson['MIExpireDate']) {
			$formattedDate =  DateTime::createFromFormat('d/m/y', $decodeJson['MIExpireDate']);
			$decodeJson['MIExpireDate'] = $formattedDate->format('d/m/Y');
		}
		if(isset($decodeJson['LocalExpireDate']) && $decodeJson['LocalExpireDate']) {
			$formattedDate =  DateTime::createFromFormat('d/m/y', $decodeJson['LocalExpireDate']);
			$decodeJson['LocalExpireDate'] = $formattedDate->format('d/m/Y');
		}
	...
	public function actionTest() {
		$sql = 'https://10.0.2.209/semuapromoapi/users/callSubscribe/20180411212802/0163662664/40932/2';
		$output = Yii::app()->curl->get($sql, $params= array(),true);
		$output  = str_replace('{},','"",',$output);
		$decodeJson = json_decode($output);
	...
	{
		// db query
		// CREATE TABLE `mytube`.`tbl_ekyc_details` ( `id` INT(11) NOT NULL AUTO_INCREMENT , `id_number` VARCHAR(250) NOT NULL , `full_name` VARCHAR(250) NOT NULL , `dob` TIMESTAMP NOT NULL , `gender` VARCHAR(20) NOT NULL , `id_type` VARCHAR(250) NOT NULL , `id_image` TEXT NOT NULL , `nationality` VARCHAR(250) NULL , `country_of_issue` VARCHAR(250) NULL , `id_expiry_date` TIMESTAMP NULL , `post_code` VARCHAR(250) NULL , `city` VARCHAR(250) NULL , `state` VARCHAR(250) NULL , `street_address` VARCHAR(250) NULL , `military_number` VARCHAR(250) NULL , `sector` VARCHAR(250) NULL , `employer` VARCHAR(250) NULL , `unique_ref_no` VARCHAR(250) NULL , `status` INT(2) NOT NULL DEFAULT '1', `created_at` TIMESTAMP NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;
		if(isset($_POST) && !empty($_POST)){
			$idImage = "";
			$idNumber = $fullName = $dob = $gender = $id_type = $nationality = $countyOfIssue = $idExpiryDate = $postCode = $city = $state = $streeAddress = $militaryNumber = $sector = $employer = "";
			$validateInfo = ["id_number","full_name","dob","gender","id_type"];
			$commonInfo = ["nationality","country_of_issue","id_expiry_date","post_code","city","state","street_address","military_number","sector","employer"];
			$validationError = [];
			foreach ($validateInfo as $key => $eachItem) {
				if(!isset($_POST[$eachItem]) && $_POST[$eachItem] != ""){
					$validationError[] = $eachItem." field is required";
				}
			}
			...
			$target_dir = "uploads/ekyc/".date("Y-m")."/";
			if (!is_dir($target_dir)) {
				mkdir($target_dir, 0777, true);
			}
			$target_file = $target_dir.date("y-m-d-h-i-s").rand(11111,99999). basename($_FILES["id_image"]["name"]);
			$uploadOk = 1;
			$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
			// Check if image file is a actual image or fake image
			if(isset($_FILES["id_image"]["tmp_name"]) && $_FILES["id_image"]["tmp_name"] != "") {
			$check = getimagesize($_FILES["id_image"]["tmp_name"]);
			if($check !== false) {
				$uploadOk = 1;
			} else {
				$this->apiError("Uploaded file is not an image");
				$uploadOk = 0;
			}
			}else{
				$this->apiError("id_image filed is required");
			}

			// Check if file already exists
			if (file_exists($target_file)) {
			$this->apiError("Sorry, the file already exists.");
			$uploadOk = 0;
			}

			// Check file size
			if ($_FILES["id_image"]["size"] > 500000) {
			$this->apiError("Sorry, your file is too large.");
			$uploadOk = 0;
			}
			...
			&& $imageFileType != "gif" ) {
			$this->apiError("Sorry, only JPG, JPEG, PNG & GIF files are allowed.");
			$uploadOk = 0;
			}

			// Check if $uploadOk is set to 0 by an error
			if ($uploadOk == 0) {
			$this->apiError("Sorry, the file was not uploaded.");
			// if everything is ok, try to upload file
			} else {
			if (move_uploaded_file($_FILES["id_image"]["tmp_name"], $target_file)) {
				//Image upload successfully				
			} else {
				$this->apiError("There was an error uploading your file.");
			}
			}
			//optional Data
			...
			$countyOfIssue = (isset($_POST['country_of_issue']))?$_POST['country_of_issue']:"";
			$idExpiryDate = (isset($_POST['id_expiry_date']))?$_POST['id_expiry_date']:"";
			$postCode = (isset($_POST['post_code']))?$_POST['post_code']:"";
			$city = (isset($_POST['city']))?$_POST['city']:"";
			$state = (isset($_POST['state']))?$_POST['state']:"";
			$streeAddress = (isset($_POST['street_address']))?$_POST['street_address']:"";
			$militaryNumber = (isset($_POST['military_number']))?$_POST['military_number']:"";
			$sector = (isset($_POST['sector']))?$_POST['sector']:"";
			$employer = (isset($_POST['employer']))?$_POST['employer']:"";
			$idImage = $target_file;
			///Insert these details on db
			$uniqueRefNo = date("Ymdhis").rand(11111,99999);
			$model = new Ekyc;
			$model->id_number = $idNumber;
			$model->full_name = $fullName;
			$model->dob = $dob;
			$model->gender = $gender;
			$model->id_type = $id_type;
			$model->id_image = $idImage;
			$model->nationality = $nationality;
			$model->country_of_issue = $countyOfIssue;
			$model->id_expiry_date = $idExpiryDate;
			$model->post_code = $postCode;
			$model->city = $city;
			$model->state = $state;
			$model->street_address = $streeAddress;
			$model->military_number = $militaryNumber;
			$model->sector = $sector;
			$model->employer = $employer;
			$model->unique_ref_no = $uniqueRefNo;
			$model->status = 1;
			$model->created_at = date("Y-m-d H:i:s");
			$check = $model->save();
			if($check){
				$data = ["success"=>true,"unique_ref_no"=>$uniqueRefNo];
				$this->data = $data;
				$this->renderJson();
			}else{
				$this->apiError("Unable to record user information");
			}
	...
	public function actionUpdateInfo(){
		if(isset($_POST["unique_ref_no"]) && isset($_POST["selected_sim_no"]) && isset($_POST["delivery_method"]) && isset($_POST["delivery_address"]) && isset($_POST["delivery_phone"])){
			if($_POST["unique_ref_no"] != "" && $_POST["selected_sim_no"] != "" && $_POST["delivery_method"] != "" && $_POST["delivery_address"] != "" && $_POST["delivery_phone"] != ""){
				$mobile = ltrim($_POST['selected_sim_no'],'6');
				$isValid =preg_match( "/^(?:601)[0-9]{8,9}$/m","6".$mobile);
				if(!$isValid){
					$this->apiError("Please provide a valid merchantrade sim number");
				}				
				$getDetails = Ekyc::model()->find('unique_ref_no=:unique_ref_no',array('unique_ref_no' => $_POST["unique_ref_no"]));
				if(!empty($getDetails)){
					if($getDetails->status == 1){
						$getDetails->status = 2;
						$getDetails->selected_sim_no = $_POST["selected_sim_no"];
						$getDetails->delivery_method = $_POST["delivery_method"];
						$getDetails->delivery_address = $_POST["delivery_address"];
						$getDetails->delivery_phone = $_POST["delivery_phone"];
						if($getDetails->save()){
							$data = ["success"=>true,"message"=>"Ekyc information updated successfully"];
							$this->data = $data;
							$this->renderJson();
						}else{
							$this->apiError("Unable to update ekyc information");
						}
					}else{
						$this->apiError("Information you are trying to update is already updated");
					}
				}else{
					$this->apiError("Provided information not matched with our records");
				}
			}else{
				$this->apiError("Required information must not be empty");
			}
		}else{
			$this->apiError("Please provide all required fiedls");
		}
	}
}