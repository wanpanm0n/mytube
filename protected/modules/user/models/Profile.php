...
return array(
	...
	public static function getLanguageId(){
		...
		if(empty($profile) && empty($profile->language_id))
			throw new ErrorException("You are not authorized!!!");
		else 
	...
	public static function getLanguage(){
		if(Yii::app()->user->isAdmin())
			return '';
		$profile = Profile::model()->findBySql("select * from tbl_profiles left join tbl_language on tbl_language.id=tbl_profiles.language_id where user_id=:uid", array(':uid'=>Yii::app()->user->id));
		if(empty($profile) && empty($profile->language_id))
			throw new ErrorException("You are not authorized!!!");
		else{
			$language = Language::model()->findByPk($profile->language_id);
			return $language->name;
		}//echo  $profile->language->name;
	}
}